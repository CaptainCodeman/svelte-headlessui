...
The typical approach is to create extra components that act as wrappers for all the parts of the UI component being implemented, so that these pieces can be coordinated behind the scenes. But I think this adds bload to the component definitions and is also inefficient.
...
Behind the scenes, the behaviors handle UI events and WAI-ARIA attributes.
